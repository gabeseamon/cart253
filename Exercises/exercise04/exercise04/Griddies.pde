...
class Griddie {
  ...
  void update() {
    ...
    //if the total energy is 0 (no more griddies) reset the griddie class
    if (energy == 0) {
      return;
    }
    ...
    // if the griddie's x or y is greater then the screen size, it appears on the oppisite side of the screen
    if (x < 0) {
      x += width;
    }
    else if (x >= width) {
  ...
  void collide(Griddie other) {
    // QUESTION: What is this if-statement for?
    // if both griddies energy is 0, they are dead (reset)
    if (energy == 0 || other.energy == 0) {
      return;
    }
    
    // QUESTION: What does this if-statement check?
    // checks if two griddies collide with each other (have the same x and y)
  ...
  void display() {
    ...
    //makes the colour equivilent to the omount of energy, so the griddies get more translucent as they have less energy
    fill(fill, energy); 
    noStroke();
    rect(x, y, size, size);
  }
